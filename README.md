...
Displays live game info on a compact always-on-top HUD: current terror,
round type, movement speed, session survivals, and round history.
Click the terror name for full stun info. Click the round type for session stats.

Desktop only.

> \\\*\\\*Work in progress.\\\*\\\* Terror data is maintained manually and may be outdated or incorrect — the game updates frequently. The current April Fools event may also cause round types and terror combinations to behave differently than the overlay expects. Trick or Treat rounds will always display wrong Map and Terror Infos. If something else is wrong, open an issue.
...
- VRChat with OSC enabled (port 9001)
...
> **Antivirus warning:** The `.exe` is compiled from Python using PyInstaller. Some antivirus programs flag PyInstaller executables as suspicious by default even when the code is completely clean. This is a known false positive with compiled Python apps. If your antivirus blocks it, you can run the `.py` source file directly instead — that requires Python 3.x installed but no other packages.
...
- Seven stun categories: Stunnable, Not Stunnable, Avoid, Do Not Stun, Partial, Conditional, Teleports
...
- Unbound round terror list (84 rounds)
- Multi-terror support for Bloodbath, Midnight, and Double Trouble
- No external dependencies — standard library only

## Notes

- Tested on Windows. Should work on any OS with Python and tkinter.
- Terror data sourced from terror.moe with manual edits.
- Panels are draggable. Close with the X button on the main overlay.
